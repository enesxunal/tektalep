...
export async function GET(
  ...
  if (!talep?.completionImageUrl || !talep.completionImageCreatedAt) {
  ...
  if (dosyaSuresiDolduMu(talep.completionImageCreatedAt)) {
    return NextResponse.json(
      {
        hata:
          "Bu dosyanın süresi doldu. Yüklemeden sonra 7 gün içinde indirilebilirdi.",
      },
      { status: 410 },
    );
  }

  const konum = saklananDosyayiAyristir(talep.completionImageUrl);

  if (konum.tur === "yerel") {
    const uzanti = konum.dosya.split(".").pop() ?? "";
    ...
    return new NextResponse(Buffer.from(tampon), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${konum.dosya}"`,
      },
    });
  }
